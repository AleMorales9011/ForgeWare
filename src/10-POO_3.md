...
An enum in Java is a special data type that allows you to define a set of named constants. These constants are like fixed values representing a specific option within a category. They are commonly used to represent fixed values, like days of the week (MONDAY, TUESDAY, WEDNESDAY) or compass directions (NORTH, SOUTH, EAST, WEST).
...
```
# Comparable vs Runnable vs Serializable
...
Allows object persistence for data storage and sharing.   
